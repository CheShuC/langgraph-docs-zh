...
LangGraph 智能体并非黑盒。每个图都由**命名节点**组成，这些节点按顺序或并行执行：分类、研究、分析、综合。图执行卡片通过为每个节点渲染一张卡片使该流水线可见：显示其状态、实时流式传输其内容，并跟踪整个工作流的完成情况。用户可以准确地看到智能体在做什么、当前处于哪个步骤，以及每个步骤产生了什么。

此模式对生产环境中的智能体尤其有用，因为它会将图结构转化为产品用户界面体验。您不必将一次运行视为单个助手的响应，而是可以暴露出 LangGraph 内部使用的相同检查点、节点名称、状态键和流元数据。

<PatternEmbed pattern="graph-execution-cards" />

## 图节点如何映射到 UI 卡片

LangGraph 图定义了一系列节点，每个节点负责一项特定任务。例如，一个研究流水线可能包含：
...
每个节点将其输出写入图状态中的特定键。在前端，您无需对映射进行硬编码，因为 [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream) 会在每个节点运行时通过 `stream.subgraphs` 发现它们，并为每个观察到的步骤暴露一个 [`SubgraphDiscoverySnapshot`](https://reference.langchain.com/javascript/langchain-react/SubgraphDiscoverySnapshot)：
...
使用 `node.nodeName` 作为进度条和卡片标题中的标签。将每个快照传递给 `useMessages(stream, node)`，以渲染节点作用域的流式内容，而无需将 UI 与图状态键名称耦合。

这种映射成为您的图与 UI 之间的契约。后端开发者可以有意识地添加、重命名或重新排序节点，而前端开发者决定每个状态键的呈现方式：状态徽章、markdown 面板、表格、图表、跟踪视图或审批卡片。

## 设置 `useStream`

照常接好 [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream)。您将使用到的关键属性是 `messages`（用于对话）和 `subgraphs`（用于当前运行中发现的图节点）。将每个发现的子图快照传递给选择器，以读取作用域到该节点的消息。
...
在图进行流式传输时，每个发现的子图快照都会标识其所属的节点。将该快照传递给选择器 Hook 或组合式函数，以读取作用域到该节点的消息：
...
每个发现的节点都携带其当前状态。直接使用 `node.status`；发现快照会报告 `"pending"`、`"running"`、`"complete"` 或 `"error"`：
...
顶部的水平进度条让用户对整个流水线一目了然。每个步骤都是一个带标签的片段，随着节点的完成而填充：
...
节点卡片会读取作用域消息，以同时获取流式内容和最终内容。这样可以避免假设图节点名称与其写入的状态键匹配（例如，playground 图中的 `do_research` 写入 `research`）：

| 来源                      | 使用时机                                                                            |
| --------------------------- | -------------------------------------------------------------------------------------- |
| `useMessages(stream, node)` | 渲染节点作用域的流式消息和最终消息                                        |
| `stream.values`             | 使用实际状态键读取整个图的状态，例如最终的 `synthesis` 字段 |

方法是：在节点卡片中显示最新的作用域 AI 消息，并且仅在您确实需要图状态字段时才使用 `stream.values`。

由于作用域消息与产生它们的节点相关联，UI 可以支持并行图路径，而无需根据消息顺序进行猜测。每张卡片都会从属于其节点的事件更新，已完成的值通过 `stream.values` 保持可用。
...
并非所有图都有固定的节点集。有些流水线会根据输入添加或跳过节点。发现映射只包含当前线程中观察到的节点：

```ts theme={"theme":{"light":"catppuccin-latte","dark":"catppuccin-mocha"}}
const activeNodes = [...stream.subgraphs.values()];
```

这可以确保您的 UI 只显示与当前执行相关的节点的卡片，避免出现空的占位卡片。

<Info>
  如果您的图有条件分支（例如，对简单的事实性查询跳过"研究"），被跳过的节点不会出现在 `stream.subgraphs` 中。您的流水线进度条可以只渲染已发现的节点，或者将没有匹配快照的预期节点变暗。
</Info>

## 最佳实践

* **从流中发现节点**。从 `stream.subgraphs` 渲染卡片，而不是硬编码预期的节点；条件步骤或被跳过的步骤在运行之前不会出现。
* **将状态键视为 UI 契约**。决定哪些图输出应该足够稳定以供前端渲染，并在图定义旁边记录这些键。
* **对节点卡片使用作用域消息**。它们在节点流式传输期间和完成后都能正常工作，而不会将 UI 卡片与状态键名称耦合。
* **自动折叠已完成的节点**。在长流水线中，自动折叠已完成的卡片，让用户专注于当前活动的步骤。
* **显示预计耗时**。如果您有每个节点耗时的历史数据，显示时间估算以设定用户的预期。
* **添加全局进度指示器**。在流水线视图顶部用整体进度条（例如"步骤 2 / 4"）补充每个节点的卡片。
* **按节点处理错误**。如果某个节点失败，在该节点的卡片中显示错误，而不要折叠整个流水线。其他节点仍可能成功完成。
