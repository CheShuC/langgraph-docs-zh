...
LangGraph 智能体流式传输的内容远不止消息和工具调用。服务端的**流转换器**可以在协议流向客户端的过程中检查或重写协议，并在命名**自定义通道**上发布自己的结构化数据。前端通过两个选择器读取该通道：[`useExtension`](https://reference.langchain.com/javascript/langchain-react/useExtension) 用于获取最新负载，[`useChannel`](https://reference.langchain.com/javascript/langchain-react/useChannel) 作为原始事件的后备方案。

下面的示例是一个客户支持智能体，其转换器会在每个事件到达浏览器之前从中删除 PII（电子邮件、电话号码、社会保障号码、卡号、IP 地址），并在 `redaction-stats` 通道上发布实时的删除统计。侧面板实时渲染这些统计。

<PatternEmbed pattern="custom-stream-channel" />

## 自定义通道如何工作

自定义通道有两个端点。在服务端，[`StreamTransformer`](https://reference.langchain.com/python/langgraph/stream/_types/StreamTransformer) 打开一个命名 [`StreamChannel`](https://reference.langchain.com/python/langgraph/stream/stream_channel/StreamChannel) 并向其推送负载。在客户端，选择器订阅匹配的 `custom:<name>` 通道，并将负载暴露为响应式状态。

转换器的 `process` 方法会对每个协议事件运行。它可以在原处修改事件（此处是从 `messages`、`tools` 和 `values` 数据中清除 PII），并在有要报告的内容时推送侧通道更新。

客户端选择器（`useExtension`、`useChannel`）随 v1 前端 SDK 包（`@langchain/react`、`@langchain/vue`、`@langchain/svelte`、`@langchain/angular`）一同提供。
...
照常接好 [`useStream`](https://reference.langchain.com/javascript/langchain-react/index/useStream)。自定义通道选择器接受此处返回的同一个 `stream` 句柄。

<Info>
  代码示例使用 `useStream<typeof myAgent>` 实现类型安全的流状态。有关 [Python](/oss/python/langchain/frontend/overview#type-inference) 或 [JavaScript](/oss/javascript/langchain/frontend/overview#type-inference) 后端的类型推断，请参阅相应文档。
</Info>
...
`useExtension` 订阅 `custom:<name>` 通道，并返回转换器推送的最新负载，该负载已解包并带有类型。当 UI 只需要当前值（例如实时计数器、进度百分比或状态徽章）时，它是更符合人体工程学的选择。

传入裸通道名称（`"redaction-stats"`），而不是 `custom:` 前缀：
...
返回值遵循每个框架的响应式模型：React 和 Svelte 中为普通值，Vue 中为 `Ref`（`latest.value`），Angular 中为信号（`latest()`）。在第一个负载到达之前，该值为 `undefined`。

可选的第三个 `target` 参数可以将订阅限定到某个命名空间，方式与 `useMessages(stream, node)` 将消息限定到已发现的图节点相同。有关命名空间定位，请参阅[图执行](/oss/python/langgraph/frontend/graph-execution)。

## 使用 `useChannel` 缓冲原始事件

`useChannel` 是原始事件的后备方案。它订阅一个或多个通道，并返回底层协议事件的有界缓冲区，而不是单个解包的值。当您需要历史记录而不是最新值（例如事件日志或审计日志），或者需要某个更高级选择器未覆盖的通道时，请使用它。
...
每个条目都是一个原始协议事件，因此负载位于 `event.params.data` 之下。您需要自己解包：
...
| `replay`     | `true`      | 在选择器挂载时重放通道上已看到的事件，而不是只重放实时事件。 |

<Note>
  对于常见情况，请优先使用更高级的选择器（`useExtension`、`useMessages`、`useToolCalls`、`useValues`）。它们返回带类型的、解包后的值，并且只跟踪您渲染的内容。当您确实需要原始事件流时，再使用 `useChannel`。
</Note>
...
| ---------------- | ---------------------------------- | -------------------------------------------------------- |
| **返回**      | 最新负载（`T \| undefined`）  | 原始事件的有界缓冲区（`Event[]`）                 |
| **形状**        | 解包后的类型化负载           | 原始协议事件；自行解包 `event.params.data` |
| **订阅方式** | 通道名称（`"redaction-stats"`） | 完整通道 ID（`["custom:redaction-stats"]`）           |
| **使用时机**     | 您需要当前值         | 您需要历史记录、日志或多个通道            |
| **选项**      | —                                  | `bufferSize`、`replay`                                   |

一种常见模式是在同一通道上同时使用两者：`useExtension` 驱动实时摘要（当前总计），而 `useChannel` 支撑跨线程每次更新的滚动事件日志。

## 使用场景

自定义通道适用于任何无法干净地映射到消息、工具调用或图状态的服务端信号：

* **合规与删除统计**：被清除的 PII、被阻止的内容或策略命中的计数，如上面的示例所示。
* **进度报告**：长时间运行的工具发出的完成百分比或步骤标签。
* **实时指标**：运行期间累积的令牌使用量、延迟或成本。
* **来源与引用**：当智能体为其答案提供依据时，将检索到的文档推送到侧面板。
* **领域事件**：后端希望在不更改消息记录的情况下呈现的任何结构化更新。

## 相关

* [概述](/oss/python/langgraph/frontend/overview) — LangGraph 前端流 API 与架构。
* [图执行](/oss/python/langgraph/frontend/graph-execution) — 用于多节点流水线的命名空间作用域选择器。
