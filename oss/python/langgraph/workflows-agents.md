...
LangGraph 在构建智能体和工作流时提供了多项优势，包括[持久化](/oss/python/langgraph/persistence)、[流式传输](/oss/python/langgraph/streaming)，以及调试和[部署](/oss/python/langgraph/deploy)支持。

<Tip>
  使用 [LangSmith](https://smith.langchain.com?utm_source=docs\&utm_medium=cta\&utm_campaign=langsmith-signup\&utm_content=oss-langgraph-workflows-agents) 追踪并比较这些工作流模式。按照[追踪快速入门](/langsmith/trace-with-langgraph)查看数据如何在每个步骤中流动。我们还建议您设置 [LangSmith Engine](/langsmith/engine)，它可以监控您的 trace、检测问题并提出修复建议。
</Tip>
...
工作流和智能体系统基于 LLM 以及您为其添加的各种增强。[工具调用](/oss/python/langchain/tools)、[结构化输出](/oss/python/langchain/structured-output)和[短期记忆](/oss/python/langchain/short-term-memory)是根据您的需求定制 LLM 的几种选择。
...
提示词链式调用（Prompt chaining）是指每个 LLM 调用都处理前一个调用的输出。它通常用于执行可以分解为更小、可验证步骤的明确定义的任务。一些例子包括：
...
通过并行化，LLM 可以同时处理一个任务。这既可以同时运行多个独立的子任务，也可以多次运行同一任务以检查不同的输出。并行化通常用于：
...
* 多次运行一个任务，根据不同标准（如引用数量、使用的来源数量、来源质量）对文档的准确性进行评分
...
路由工作流处理输入，然后将其引导到特定于上下文的任务。这让您可以为复杂任务定义专门的流程。例如，一个用于回答产品相关问题的工作流可能会先处理问题类型，然后将请求路由到定价、退款、退货等特定流程。
...
在编排器-工作器（Orchestrator-worker）配置中，编排器：
...
编排器-工作器工作流提供了更大的灵活性，当子任务无法像[并行化](#parallelization)那样预定义时，通常使用这种模式。这在编写代码或需要跨多个文件更新内容的工作流中很常见。例如，一个需要在未知数量的文档中为多个 Python 库更新安装说明的工作流可能会使用这种模式。
...
编排器-工作器工作流很常见，LangGraph 对其有内置支持。`Send` API 让您可以动态创建工作器节点并向它们发送特定输入。每个工作器都有自己的状态，所有工作器输出都写入一个共享的状态键，该键对编排器图可见。这使编排器可以访问所有工作器输出，并将其合成为最终输出。下面的示例遍历一个章节列表，并使用 `Send` API 向每个工作器发送一个章节。
...
在评估器-优化器（Evaluator-optimizer）工作流中，一个 LLM 调用创建响应，另一个评估该响应。如果评估器或[人机协同](/oss/python/langgraph/interrupts)确定响应需要改进，则会提供反馈并重新创建响应。这个循环一直持续到生成可接受的响应为止。

当任务有特定的成功标准，但需要迭代才能达到该标准时，通常使用评估器-优化器工作流。例如，在两种语言之间翻译文本时并不总能完美匹配。可能需要几次迭代才能生成在两种语言中具有相同含义的翻译。
...
智能体通常被实现为使用[工具](/oss/python/langchain/tools)执行操作的 LLM。它们在持续的反馈循环中运行，用于问题和解决方案不可预测的场景。智能体比工作流拥有更多自主性，可以决定使用哪些工具以及如何解决问题。您仍然可以定义可用的工具集以及智能体行为的准则。
...
<Note>
  要开始使用智能体，请参阅[快速入门](/oss/python/langchain/quickstart)，或在 LangChain 中阅读更多关于[它们如何工作](/oss/python/langchain/agents)的内容。
</Note>
...
当您需要对图的工具执行方式进行细粒度控制时，请使用 [`ToolNode`](https://reference.langchain.com/python/langgraph/agents/#langgraph.prebuilt.tool_node.ToolNode)。它是许多 LangGraph 智能体模式中驱动工具执行的基础构件。
...
由 `ToolNode` 执行的工具将模型生成的参数作为其第一个参数接收。要读取不是由模型生成的图侧数据，请使用以下选项之一：

* 在 Python 中，从注入的 [`ToolRuntime`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.ToolRuntime) 参数读取状态和运行范围的上下文。
* 在 JavaScript 中，从工具的第二个参数读取状态和运行范围的上下文，其类型为 [`ToolRuntime`](https://reference.langchain.com/python/langchain/tools/#langchain.tools.ToolRuntime)。

<Note>
  工具只能访问传递给 `ToolNode` 的状态值。当 `ToolNode` 直接作为 `StateGraph` 节点添加时，该输入就是当前的图状态。如果您从另一个节点手动调用 `ToolNode`，请在工具需要自定义状态字段时传递完整状态。例如，`tool_node.invoke(state)` 或 `toolNode.invoke(state, config)` 暴露完整状态，而只传递 `{"messages": state["messages"]}` 或 `{ messages: state.messages }` 只会暴露 `messages`。
</Note>
